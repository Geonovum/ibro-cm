...
| `Overig bouwwerk` type `overkapping` | `Pand` type `Overkapping` | |
| `Overig bouwwerk` type `open loods` | `Pand` type `loods` met `indicatieOpenConstructie` is `true` | |
| `Overig bouwwerk` type `bezinkbak` | `Reservoir` type `Bezinkbak` | |
| `Overig bouwwerk` type `lage trafo` | `Straatmeubilair` type `Kast` met bijbehorende `Installatie` type `Transformator` | |
| `Overig bouwwerk` type `bassin` | `Reservoir` type `Bassin`, of `Dok`, of `Zwembad` | |
